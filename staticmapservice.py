...
def process_icon(i, pnv):
    ...
    cache_key = f"{i_lon},{i_lat}-{i_name}-{i_offset_x}-{i_offset_y}_{pnv}"

    icon_marker = icon_markers_cache.get(cache_key, None)
    if not icon_marker:
        icon_marker = (
            IconMarker(
                (i_lon, i_lat), "./icons/{0}.png".format(i_name), i_offset_x, i_offset_y
            ),
            pnv,
        )
        icon_markers_cache[cache_key] = icon_marker
